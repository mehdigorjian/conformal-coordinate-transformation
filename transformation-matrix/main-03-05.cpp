...
void calculateTransformMatrix(Eigen::MatrixXd& transformMat, double& phi, double& psi, double& theta, Eigen::MatrixXd& A, Eigen::MatrixXd& C1, Eigen::MatrixXd& C2, Eigen::MatrixXd& C3, Eigen::Vector3d* ptsA, Eigen::Vector3d* ptsB, int numOfptsPairs) {
...
void generatedPointsEquivaletPtsB(Eigen::MatrixXd& transformMat, Eigen::Vector3d* ptsA, Eigen::Vector3d* ptsB, Eigen::Vector3d* ptsBcalculated, int numOfptsPairs) {
    for (int i = 0; i < numOfptsPairs; i++) {
        Eigen::Vector3d calcPt;
        transformPointFromSysAtoSysB(transformMat, ptsA[i], calcPt);
        ptsBcalculated[i] = calcPt;
    }
}

double errorCalculation(Eigen::Vector3d* ptsB, Eigen::Vector3d* ptsBcalculated, int numOfptsPairs) {
    double sum = 0.0;
    for (int i = 0; i < numOfptsPairs; i++) {
...
int main(int argc, char* argv[]) {
    ...
    Eigen::Vector3d ptsBcalculated[numOfptsPairs];

    std::cout << "----------Transformation Matrix----------" << std::endl;

    calculateTransformMatrix(transformMat, phi, psi, theta, A, C1, C2, C3, ptsA, ptsB, numOfptsPairs);
    ...
    generatedPointsEquivaletPtsB(transformMat, ptsA, ptsB, ptsBcalculated, numOfptsPairs);
    // for (int i = 0; i < numOfptsPairs; i++) {
    //     std::cout << "----------" << std::endl;
    ...
    std::cout << errorCalculation(ptsB, ptsBcalculated, numOfptsPairs) << std::endl;
