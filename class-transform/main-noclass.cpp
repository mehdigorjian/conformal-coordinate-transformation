...
void calculateA(Eigen::MatrixXd& A, std::vector<Eigen::Vector3d>& ptsA) {
...
void calculateC1(Eigen::MatrixXd& C1, std::vector<Eigen::Vector3d>& ptsA, std::vector<Eigen::Vector3d>& ptsB) {
    for (int i = 0; i < ptsA.size(); i++) {
...
void calculateC2(Eigen::MatrixXd& C2, std::vector<Eigen::Vector3d>& ptsA, std::vector<Eigen::Vector3d>& ptsB) {
    for (int i = 0; i < ptsA.size(); i++) {
...
void calculateC3(Eigen::MatrixXd& C3, std::vector<Eigen::Vector3d>& ptsA, std::vector<Eigen::Vector3d>& ptsB) {
    for (int i = 0; i < ptsA.size(); i++) {
...
void calculateTransformMatrix(Eigen::MatrixXd& transformMat, double& phi, double& psi, double& theta, Eigen::MatrixXd& A, Eigen::MatrixXd& C1, Eigen::MatrixXd& C2, Eigen::MatrixXd& C3, std::vector<Eigen::Vector3d>& ptsA, std::vector<Eigen::Vector3d>& ptsB) {
    calculateA(A, ptsA);
    calculateC1(C1, ptsA, ptsB);
    calculateC2(C2, ptsA, ptsB);
    calculateC3(C3, ptsA, ptsB);
...
void transformPointFromSysAtoSysB(Eigen::MatrixXd& transformMat, Eigen::Vector3d& pt, Eigen::Vector3d& transformedPt) {
...
void generatedPointsEquivaletPtsB(Eigen::MatrixXd& transformMat, std::vector<Eigen::Vector3d>& ptsA, std::vector<Eigen::Vector3d>& ptsB, std::vector<Eigen::Vector3d>& ptsBcalculated) {
    for (int i = 0; i < ptsA.size(); i++) {
        Eigen::Vector3d calcPt;
        transformPointFromSysAtoSysB(transformMat, ptsA[i], calcPt);
        ptsBcalculated.push_back(calcPt);
    }
}

double errorCalculation(std::vector<Eigen::Vector3d>& ptsB, std::vector<Eigen::Vector3d>& ptsBcalculated) {
    double sum = 0.0;
...
int main(int argc, char* argv[]) {
    ...
    calculateTransformMatrix(transformMat, phi, psi, theta, A, C1, C2, C3, ptsA, ptsB);
    ...
    // Eigen::Vector3d pp;
    // transformPointFromSysAtoSysB(transformMat, p1c, pp);
    // std::cout << pp << std::endl;

    generatedPointsEquivaletPtsB(transformMat, ptsA, ptsB, ptsBcalculated);
    // for (int i = 0; i < ptsB.size(); i++) {
    //     std::cout << "----------" << std::endl;
    //     std::cout << ptsBcalculated[i] << std::endl;
    // }

    std::cout << "----------Error----------" << std::endl;
    std::cout << errorCalculation(ptsB, ptsBcalculated) << std::endl;
